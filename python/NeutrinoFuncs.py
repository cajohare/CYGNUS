...
def NuRate(E_r,t,Expt,NuBG,nu_i): # Time-Energy
    ...
    if Expt.Directional:
        ...
        dR = zeros(shape=(ne))
        if Solar[nu_i]:
            dR = dRdEdO_solarnu(E_r,t,E_nu_all[:,nu_i],Flux_all[:,nu_i],Nuc,Loc)
        else:
            dR = dRdEdO_isonu(E_r,E_nu_all[:,nu_i],Flux_all[:,nu_i],Nuc)
    else:
        ne = size(E_r)
        dR = zeros(shape=(ne))
        dR = dRdE_nu(E_r,t,Solar[nu_i],E_nu_all[:,nu_i],Flux_all[:,nu_i],Nuc)
    return dR
