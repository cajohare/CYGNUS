...
!------------------------Logarithmically Spaced Array---------------------------------!
  function logspace(m_lower,m_upper,n_m_vals) result(m_vals)
    double precision :: m_vals(n_m_vals),m_upper,m_lower,wid
    integer :: n_m_vals,i_m
    ! m_chi values
    wid = (log10(m_upper)-log10(m_lower))/(1.0d0*n_m_vals-1.0d0)
    do i_m = 1,n_m_vals
      m_vals(i_m) = m_lower*10.0d0**((i_m-1)*wid)
    end do
  end function
...
!-----------------------complex error function----------------------------------------!
	...
	! Dawson's function approximation.
